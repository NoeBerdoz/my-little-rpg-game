...
public class Shooter : MonoBehaviour, IEnemy
{
    ...
    private IEnumerator ShootRoutine()
    {
        ...
        for (int i = 0; i < burstCount; i++)
        {
            ...
            // Redefine target position when oscillation goes back
            if (oscillate && i % 2 != 1)
            {
                TargetConeOfInfluence(out startAngle, out currentAngle, out angleStep, out endAngle);
            } else if (oscillate)
            ...
            for (int j = 0; j < projectilesPerBurst; j++)
            {
                ...
                if (newBullet.TryGetComponent(out Projectile projectile)) // 'out' make it similar to what does GetComponent<T>
                {
                    projectile.UpdateSpeedMoveRange(bulletMoveSpeed);
                }
            ...
            // Keep shooting when stagger is on 
